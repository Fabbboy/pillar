...
void pillar_vec_init_with(struct Pillar_Vec<T> *vec,
                          struct Pillar_Allocator allocator, pil_usize cap) {
...
struct Pillar_Status pillar_vec_append(struct Pillar_Vec<T> *vec, T *item) {
  return pillar_raw_vec_append(&vec->raw, (pil_u8 *)item);
}

template <typename T>
struct Pillar_Status pillar_vec_pop(struct Pillar_Vec<T> *vec, T *out) {
  return pillar_raw_vec_pop(&vec->raw, (pil_u8 *)out);
}

template <typename T>
struct Pillar_Status pillar_vec_prepend(struct Pillar_Vec<T> *vec, T *item) {
  return pillar_raw_vec_prepend(&vec->raw, (pil_u8 *)item);
}

template <typename T>
struct Pillar_Status pillar_vec_shift(struct Pillar_Vec<T> *vec, T *out) {
  return pillar_raw_vec_shift(&vec->raw, (pil_u8 *)out);
}

template <typename T>
struct Pillar_Status pillar_vec_at(struct Pillar_Vec<T> *vec, pil_usize index,
                                   T **out) {
  pil_u8 *ptr = NULL;
